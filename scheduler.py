...
def start_scheduler(app):
    ...
    # Hourly: check trip assignment timeouts
    _scheduler.add_job(
...
def check_document_expiry():
    ...
    try:
        ...
        # Block vehicles on compliance hold
        conn.execute("""
...
def _queue_expiry_notification(conn, table, row, days_left, status):
    """Insert in-app notification for document expiry."""
    import uuid
    ...
    # Notify only at exact thresholds to avoid daily spam
...
def check_trip_timeouts():
    """Return assigned trips to 'posted' if transporter didn't respond in 4 business hours."""
    from database import get_db
    from datetime import datetime
