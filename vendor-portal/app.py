...
from auth import (
    ...
    transporter_required, audit, new_id, ROLE_PORTAL
)
...
def api_verify_otp():
    ...
    resp.set_cookie("portal_session", token, httponly=True, samesite="Lax", max_age=1800)
    return resp
...
def api_me():
    ...
    try:
        if u["entity_type"] == "vendor" and u["entity_id"]:
            row = conn.execute("SELECT contact_name FROM vendors WHERE id=?", (u["entity_id"],)).fetchone()
            if row: name = row["contact_name"]
        elif u["entity_type"] == "transporter" and u["entity_id"]:
            row = conn.execute("SELECT contact_name FROM transporters WHERE id=?", (u["entity_id"],)).fetchone()
            if row: name = row["contact_name"]
    finally:
        conn.close()
    return jsonify({
...
def api_vendor_register_init():
    ...
    try:
        ...
        conn.execute(
            "INSERT INTO users(id,mobile,email,role,entity_type,entity_id,is_active,created_at,updated_at)"
...
def api_vendor_verify_otp():
    ...
    resp.set_cookie("portal_session", token, httponly=True, samesite="Lax", max_age=7200)
    return resp
...
def api_transporter_register_init():
    ...
    try:
        ...
        return jsonify({
            "message": "OTP sent",
            "user_id": user_id, "transporter_id": transporter_id,
            "dev_otp": otp if os.environ.get("FLASK_ENV") == "development" else None
        }), 201
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def api_notifications():
    ...
    try:
        rows = conn.execute(
            "SELECT id,title,body,event_type,entity_type,entity_id,is_read,created_at"
...
def api_mark_read(nid):
    conn = get_db()
    try:
        conn.execute("UPDATE notifications SET is_read=1 WHERE id=? AND user_id=?", (nid, g.user["id"]))
        conn.commit()
    finally:
        conn.close()
...
def api_setup_admin():
    conn = get_db()
    try:
        if conn.execute("SELECT id FROM users WHERE role='super_admin' LIMIT 1").fetchone():
...
def api_vendor_dashboard():
    ...
    try:
        ...
        open_rfqs = conn.execute(
            "SELECT COUNT(*) as n FROM rfqs r WHERE r.status='published' AND r.submission_deadline>?"
            " AND (r.target_type='open' OR EXISTS(SELECT 1 FROM rfq_targeted_vendors WHERE rfq_id=r.id AND vendor_id=?))",
            (now, vendor_id)
        ).fetchone()["n"]
        ...
        docs_expiring = conn.execute(
            ...
            " WHERE vendor_id=? AND expiry_status IN ('expiring_7','expiring_15','expiring_30','expired') AND is_deleted=0",
            (vendor_id,)
        ).fetchone()["n"]
        ...
        return jsonify({
            "open_rfqs": open_rfqs, "active_quotes": active_quotes,
            "pending_ack": pending_ack, "pending_invoices": pending_inv,
            "payments_month": payments_month, "docs_expiring": docs_expiring,
...
def api_transporter_dashboard():
    ...
    try:
        ...
        active_trips = conn.execute(
            "SELECT COUNT(*) as n FROM trips WHERE transporter_id=? AND status IN ('accepted','loaded','in_transit')",
            (tid,)
        ).fetchone()["n"]
        ...
        compliance_alerts = conn.execute(
            "SELECT COUNT(*) as n FROM vehicle_documents vd JOIN vehicles v ON vd.vehicle_id=v.id"
            " WHERE v.transporter_id=? AND vd.expiry_status IN ('expired','expiring_7','expiring_15') AND vd.is_deleted=0",
            (tid,)
        ).fetchone()["n"]
        ...
        return jsonify({
            "pending_assignments": pending_assign, "active_trips": active_trips,
            "pod_pending": pod_pending, "invoice_pending": inv_pending,
            "payments_month": payments_month, "compliance_alerts": compliance_alerts,
...
def api_admin_dashboard():
    ...
    try:
        ...
        open_rfqs = conn.execute("SELECT COUNT(*) as n FROM rfqs WHERE status='published'").fetchone()["n"]
        quotes_eval = conn.execute(
            "SELECT COUNT(DISTINCT rfq_id) as n FROM quotes WHERE status='submitted' AND is_active=1"
        ...
        inv_review = conn.execute("SELECT COUNT(*) as n FROM invoices WHERE status='submitted'").fetchone()["n"]
        compliance = conn.execute(
            "SELECT COUNT(*) as n FROM ("
            " SELECT id FROM vendor_documents WHERE expiry_status IN ('expired','expiring_7') AND is_deleted=0"
            " UNION ALL SELECT id FROM transporter_documents WHERE expiry_status IN ('expired','expiring_7') AND is_deleted=0"
            " UNION ALL SELECT id FROM vehicle_documents WHERE expiry_status IN ('expired','expiring_7') AND is_deleted=0)"
        ).fetchone()["n"]
        recent = conn.execute(
        ...
        return jsonify({
            "pending_registrations": pending_reg, "open_rfqs": open_rfqs,
            "quotes_awaiting_eval": quotes_eval, "pos_awaiting_grn": pos_grn,
            "invoices_under_review": inv_review, "compliance_alerts": compliance,
